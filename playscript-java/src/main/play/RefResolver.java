...
public class RefResolver extends PlayScriptBaseListener {
     ...
     *      1、递归获取当前节点的闭包scope
     *      2、如果scope是一个代码块blockScope，则进行递归消解
     */
    @Override
    ...
    /**     退出primary表达式时触发
     *      1、获取此节点的闭包scope
     *      2、如果是ID，则判断其是变量还是函数，然后设置此节点类型为对应的函数或者变量
     *      3、如果是字面量、this、that都需要找到其对应的类型后设置此节点的类型
     *
     */
    @Override
    ...
    /**     退出函数调用节点时触发
     *      1、如果是this、super调用，接入到thisConstructorLis、superConstructorList
     *      2、如果是id调用、获取其变量名称和参数列表。开始逐级寻找其能够匹配的函数
     *      3、如果是类的调用，则在类上面逐级递归查找对应的方法
     *      4、如果不是类的调用，则递归在此节点的闭包scope中查找对应的方法
     */
    @Override
